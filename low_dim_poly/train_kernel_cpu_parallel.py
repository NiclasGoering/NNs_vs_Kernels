...
def create_neural_tangent_model(d: int, hidden_size: int, depth: int, key):
    """Create a neural tangent kernel model with specific initialization"""
    layers = []
    for _ in range(depth):
        layers.extend([
            nt.stax.Dense(hidden_size, W_std=1.0, b_std=0.1, key=key), 
            nt.stax.Relu()
        ])
    layers.append(nt.stax.Dense(1, W_std=1.0, b_std=0.1, key=key))
...
def main():
    ...
    experiment_name = "low_dim_poly_kernels_1612"
    ...
    if rank == 0:
        max_total_size = max(int(max(n_train_sizes) / 0.8))
        X_full, y_full, U, coeff_vec = generate_latent_poly_data(
            n_samples=max_total_size,
            ambient_dim=ambient_dim,
            latent_dim=latent_dim,
            degree=degree,
            noise_std=noise_std,
    ...
    # Broadcast full dataset to all workers
    ...
    for params in worker_combinations:
        ...
        # Calculate sizes and create train/test split
        n_total = int(params['n_train'] / 0.8)
        n_train = params['n_train']
        
        # Use subset of the full data
        X_train = X_full[:n_train]
        y_train = y_full[:n_train]
        X_test = X_full[n_train:n_total]
        y_test = y_full[n_train:n_total]
        ...
        try:
            ...
            result = {
                ...
                'depth': params['depth'],
                'n_train': params['n_train'],
                'n_test': X_test.shape[0] if 'X_test' in locals() else None,
                'training_mode': params['training_mode'],
                'test_error': None,
                'status': 'failed',
                'error': str(e),
