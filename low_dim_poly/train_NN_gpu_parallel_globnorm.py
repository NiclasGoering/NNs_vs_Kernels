...
def generate_and_normalize_data(n_train, X_test_fixed, y_test_fixed, ambient_dim, latent_dim, degree, global_mean, global_std, noise_std=0.1, shuffle=False, random_state=None):
    """
    Generate training data and normalize both train and test sets using global statistics.
    If shuffle is True, replace labels with Gaussian noise matching the original mean and variance.
    """
...
def main():
    ...
    # Parameters
    experiment_name = "low_dim_poly_prune_2801_false_big"
    ambient_dim = 20
    latent_dim = 3
    degree = 5
    noise_std = 0.0
