...
print(f"Process {rank}/{size} initialized")
comm.Barrier()  # Make sure all processes print before continuing
if rank == 0:
    print(f"\nTotal number of MPI processes: {size}\n")
    print("-" * 50)
...
def poisson_sample(min_radius, max_radius, mean, device):
    shape = mean
    scale = 1.0
    gamma = torch.distributions.Gamma(torch.tensor([shape], device=device), 
                                    torch.tensor([scale], device=device))
    sample = gamma.sample()
    return torch.clamp(sample, min_radius, max_radius).item()
...
def evaluate_vector(model, vector, current_params, X_train, y_train, device):
    idx = 0
...
def sphere_search_optimization_mpi(model, X_train, y_train, X_test, y_test, num_iterations, num_vectors, 
                                min_radius, max_radius, start_mean, min_mean, max_mean, experiment_name,
                                history_length):
    ...
    if rank == 0:
        ...
        print("-" * 50)
        radius_history = deque([start_mean], maxlen=history_length)
        metrics = {
            'train_losses': [],
            'test_losses': [],
            'mean_radii': [],
    ...
    current_mean = comm.bcast(start_mean if rank == 0 else None, root=0)
    ...
    #print(f"Process {rank} will handle {vectors_per_process} vectors per iteration")
    comm.Barrier()
    ...
    for iteration in range(num_iterations):
        ...
        local_best_radius = None

        for vec_idx in range(vectors_per_process):
            #if vec_idx % (vectors_per_process // 5) == 0:  # Print progress 5 times per iteration
            #    print(f"Process {rank}: Processing vector {vec_idx}/{vectors_per_process} in iteration {iteration + 1}")
            
            radius = poisson_sample(min_radius, max_radius, current_mean, device)
            vector = simple_random_sphere(num_params, radius, device)
            loss = evaluate_vector(model, vector, current_params, X_train, y_train, device)

            if loss < current_best_loss:
                local_best_loss = loss
                local_best_vector = vector.clone()
                local_best_radius = radius

        #print(f"Process {rank} completed iteration {iteration + 1} with best loss: {local_best_loss:.8f}")
        
        # Gather results
        all_losses = comm.gather(local_best_loss, root=0)
        all_radii = comm.gather(local_best_radius, root=0)
        ...
        if rank == 0:
            ...
            if valid_losses:
                ...
                #print(f"\nBest losses from all processes: {[f'{loss:.8f}' for loss in valid_losses]}")
            else:
                should_update = False
                best_idx = -1
        ...
        if should_update:
            if rank == 0:
                print(f"ohmo better solution in process {best_idx}")
                iteration_best_vector = torch.from_numpy(all_vectors_numpy[best_idx]).to(device)
                iteration_best_radius = all_radii[best_idx]
                
                radius_history.append(iteration_best_radius)
                current_best_loss = iteration_best_loss
                current_mean = np.mean(radius_history)
                current_mean = max(min_mean, min(max_mean, current_mean))
            ...
            current_mean = comm.bcast(current_mean if rank == 0 else None, root=0)
        ...
        if rank == 0:
            ...
            metrics['mean_radii'].append(current_mean)
            metrics['radius_history'].append(list(radius_history))
            ...
            if iteration % 1 == 0:
                ...
                print(f'Current Mean Radius: {current_mean:.4f}')
                print('-' * 50)
            
            # More detailed logging at checkpoints
            if iteration % 100 == 0:
                ...
                print(f'Radius History: {list(radius_history)}')
                print(f'Current Mean Radius: {current_mean:.4f}')
                ...
                checkpoint = {
                    ...
                    'metrics': metrics,
                    'radius_history': list(radius_history)
                }
...
def main():
    ...
    max_mean = 25.0
    history_length = 10
    experiment_name = "msp_sphere_search_500k_2_25"
