...
def compute_and_save_spectrum(K: np.ndarray, k: int, save_path: str):
    ...
    eigenvals, _ = eigh(K, eigvals=(n-k, n-1))
...
def train_and_evaluate(model: nn.Module, 
    ...
    """Train with gradient accumulation and compute conjugate kernels"""
    ...
    with torch.no_grad():
        ...
        for layer_idx, activations in enumerate(initial_activations):
            ...
            k = min(2000, K.shape[0])  # Use 2k eigenvalues or matrix size if smaller
    ...
    for epoch in range(epochs):
        ...
        if epoch % 500 == 0:
            ...
            # Compute conjugate kernels periodically
            print(f"Computing conjugate kernels at epoch {epoch}...")
            activations = compute_layer_activations(model, X_train)
            for layer_idx, layer_activations in enumerate(activations):
                print(f"Computing conjugate kernel for layer {layer_idx+1}")
                K = compute_conjugate_kernel_batched(layer_activations)
                
                # Compute and save spectrum
                k = min(2000, K.shape[0])
                spectrum_path = os.path.join(
                    results_dir, 
                    f'spectrum_epoch{epoch}_layer{layer_idx+1}_h{model.hidden_size}_d{model.depth}_n{len(X_train)}_lr{lr}_{timestamp}_rank{rank}.npy'
                )
                compute_and_save_spectrum(K, k, spectrum_path)
