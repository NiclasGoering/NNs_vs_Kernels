...
def generate_latent_poly_data(n_samples, ambient_dim, latent_dim, degree, noise_std=0.0, random_state=None):
    ...
    # Add noise (though we're using noise_std=0)
    if noise_std > 0:
...
# Initialize model
model = DeepNet(input_dim=ambient_dim, hidden_dim=1000)
...
for epoch in range(num_epochs):
    ...
    # Early stopping check - if we achieve very low training error
    if train_loss < 1e-8:
        print(f"Achieved very low training error ({train_loss:.2e}). Stopping early.")
        break
...
print(f'\nFinal Results:')
print(f'Training MSE: {final_train_loss:.8f}')
print(f'Test MSE: {final_test_loss:.8f}')