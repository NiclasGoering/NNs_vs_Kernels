...
def compute_empirical_ntk(kernel_fn: Any, params: Any, x1: jnp.ndarray, 
                         x2: jnp.ndarray = None) -> jnp.ndarray:
    """Compute empirical NTK"""
    if x2 is None:
        x2 = x1
    return kernel_fn(x1, x2, 'ntk', params)
...
def train_and_evaluate(init_fn: Any, apply_fn: Any, msp: MSPFunction,
    ...
    # Create optimizer chain with gradient clipping and AdamW
    ...
    kernel_fn = nt.empirical_kernel_fn(apply_fn, vmap_axes=0)
    
    # Compute initial empirical NTK
    try:
        initial_ntk = compute_empirical_ntk(kernel_fn, params, X_train, X_train)
        initial_ntk_test = compute_empirical_ntk(kernel_fn, params, X_test, X_train)
    ...
    try:
        final_ntk = compute_empirical_ntk(kernel_fn, params, X_train, X_train)
        final_ntk_test = compute_empirical_ntk(kernel_fn, params, X_test, X_train)
...
def main():
    ...
    data_dir = results_dir #os.path.join(base_path, "stair_function/data", experiment_name)
    ...
    for params in worker_combinations:
        ...
        try:
            (final_params, best_test_error, training_history,
             ...
             init_ntk, final_ntk) = train_and_evaluate(
